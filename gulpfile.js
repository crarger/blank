var gulp = require('gulp'), // Подключаем Gulp
    ...
    jimpResize = require('gulp-jimp-resize'), // Подключаем библиотеку для изменения размера изображений
...
gulp.task('export-to-dist', function () {
    var
        ...
        buildDocs = gulp.src(path.sources.json) // Переносим json в продакшен
            .pipe(gulp.dest(path.dist.docs));
});
...
gulp.task('watch', ['clean', 'browser-sync', 'sass', 'html-dist', 'export-to-dist', 'img-min', 'fontmin', 'js-dist'], function () {
...
gulp.task('img-resize', function() {
    return gulp.src(path.sources.img)
        .pipe(jimpResize({
            sizes: [
                {"width": 1920}
            ]
...
gulp.task('build-export-to-dist', function () {
    var
        ...
        buildDocs = gulp.src(path.sources.docs) // Переносим json в продакшен
...
gulp.task('build', ['build-clean', 'build-sass', 'html-build', 'build-export-to-dist', 'build-img-min', 'build-fontmin', 'js-build']);